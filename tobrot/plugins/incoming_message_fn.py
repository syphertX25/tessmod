...
async def incoming_message_f(client, message):
    ...
    if rep_mess:
        if not rep_mess.media or rep_mess.document.file_name.lower().endswith(".torrent"):
            ...
            if user_command == LEECH_COMMAND.lower():
                await i_m_sefg.edit("Don't Spam, Provide a Source to Leech")
                return
            is_file = True
            dl_url = rep_mess
    ...
    if dl_url is not None:
        ...
        aria_i_p = ''
        if not is_file:
            await i_m_sefg.edit_text("⚡𝙚𝙭𝙩𝙧𝙖𝙘𝙩𝙞𝙣𝙜 𝙡𝙞𝙣𝙠s")
...
async def g_yt_playlist(client, message):
    ...
    if message.reply_to_message:
        ...
        await message.reply_text("Don't Spam, Provide a Source to Leech", quote=True)
        return
...
async def rename_tg_file(client, message):
    ...
    if not message.reply_to_message:
        await message.reply("Don't Spam, Provide a Source to Leech", quote=True)
        return
